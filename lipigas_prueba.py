...
def updateBBDDVolumen(numero,volumen,porcentaje_volumen):
...
def updateBBDD(numero,volumen_acumulado,contador,tiempo_llenado):
...
def insertBBDD(altura_raw,estanque_id,volumen_normalizado,altura_normalizada,volumen_raw):
...
def volumen_actual_medido(volumen_porcentaje,estanque_modelo_id):
...
def variacion_porcentual_medido(porcentaje2,porcentaje1):
...
def relleno_estanque(porcentaje2,porcentaje1,estanque_estado_id,estanque_id,estanque_modelo_id):
    ...
    if conn:
        ...
        cursor = conn.cursor()
        print "obteniendo tupla"
        tiempo_llenado_estanque = tiempo_llenado(estanque_id)# el tiempo que se demora en hacer una carga del estanque
        ...
        if estanque_estado_id != 2:
            if porcentaje2 is not None:
                ...
                volumen_medido = volumen_actual_medido(porcentaje2,estanque_modelo_id)#volumen actual medido m3
                variacion_porcentual = variacion_porcentual_medido(porcentaje2,porcentaje1) # variacion de % 
                ...
                if variacion_porcentual > 1.5:
                    ...
                    if contador_primer_volumen == 1:
                        ...
                        update=updateBBDD(estanque_id,volumen_acumulado,contador_espera,tiempo_llenado_estanque)
